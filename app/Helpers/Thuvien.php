...
function tv_mahoa($mess)
{
    $encryptionMethod = "AES-256-CBC";
    $secret = "Ks5zPKtfqKJG21Bzplu5JSgn59HVXZ4E";  //phai 32 ky tu
    $iv = substr($secret, 0, 16);
    return openssl_encrypt($mess, $encryptionMethod, $secret, 0, $iv);
}

function tv_taochuoi()
{
    $captcha_num = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890abcdefghjkmnopqrstuvwxyz';
    $str = substr(str_shuffle($captcha_num), 0, 6);
    return $str;
}
function catchuoi($value, $limit, $end = '')
{
...
function khongdau($str, $strSymbol = '-', $case = MB_CASE_LOWER)
{
    $str = trim($str);
    if ($str == "") return "";
    $str = str_replace('"', '', $str);
...
function resize($path, $name, $w, $h)
{
    $image = imagecreatefromjpeg($path . $name);
    $img = imagescale($image, $w, $h);
